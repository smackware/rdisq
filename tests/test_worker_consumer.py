...
def test_simple_positive(simple_worker):
    ...
    assert SimpleWorker.get_consumer().build("a house")['message from the worker'] == "I'm done!"
    assert SimpleWorker.get_consumer().build("a house", "hammer")['message from the worker'] == "I'm done!"
...
def test_complex_positive(complex_worker):
    ...
    async_reply = async_consumer.calculate(1, 2, 3)
    assert async_reply.wait() == ComplexWorker.calculate(1, 2, 3)
